...
%                   loop for finding the "best" penalty c value in each
%                   iteration of the outer Cross-Validation loop of.
...
if size(Y,1) ~= n
    error('The number of rows in the X and Y should be equal.');
end
if size(Y,2) ~= 1
...
for i = 1 : k
    ...
    for j = 1 : k
        if k-i+1 ~= j
            ...
            if ~isempty(weights)
                w(currsize+1 : currsize+1+sizes(j)-1, 1) = weights(ind(offsets(j):offsets(j)+sizes(j)-1), 1);
            end
    ...
    if isempty(cvc_k)
        ...
        [model, time(i)] = aresbuild(Xtr, Ytr, trainParams, w, false, [], verbose);
    else
        % perform finding the "best" value for trainParams.c and then build
        % a model using the "best" value
        tic;
        trainParams.c = arescvc(Xtr, Ytr, trainParams, cvc_cTry, w, cvc_k, false, false);
        model = aresbuild(Xtr, Ytr, trainParams, w, false, [], verbose);
        time(i) = toc;
    end

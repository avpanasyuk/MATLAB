classdef device < handle
  ...
  methods
    ...
    function a = device()
      ...
      display(['Version: ' AVP.HW.AD.device.GetVersion()]);

      a.h = AVP.HW.AD.device.DeviceOpen();
    ...
    function varargout = CallByName(a,name,varargin)
      %> calls DWF function with the same name as a calling member function
      [varargout{1:nargout}] = ...
        AVP.HW.AD.device.StaticCallByName(name,a.h,varargin{:});
    end % Call

    function value = GetValueByName(a, name, type)
      %> calls function which takes pointer to type and returns a single value
      p = libpointer([type 'Ptr'],0);
      AVP.HW.AD.device.StaticCallByName([name, 'Get'],a.h,p);
      value = p.Value;
    end

    function SetValueByName(a, name, value)
      %> calls function which takes pointer to type and returns a single value
      AVP.HW.AD.device.StaticCallByName([name, 'Set'],a.h,value);
    end
  ...
  methods (Access = protected)
    function value = GetValue(a, type)
      %> calls function which takes pointer to type and returns a single value
      p = libpointer([type 'Ptr'],0);
      AVP.HW.AD.device.StaticCall_level(3,a.h,p);
      value = p.Value;
    end
  ...
  methods(Static)
    ...
    function Num = Enum()
      ...
      AVP.HW.AD.device.StaticCall(2,p)
      Num = p.Value;
    end

    function h = DeviceOpen()
      p = libpointer('int32Ptr',0);
      AVP.HW.AD.device.StaticCall(-1,p);
      h = p.Value;
    end % DeviceOpen
  end % static methods
  ...
  methods(Static, Access = protected)
    function varargout = StaticCall(varargin)
      [varargout{1:nargout}] = ...
        AVP.HW.AD.device.StaticCall_level(3,varargin{:});
    end % StaticCall
    ...
    function varargout = StaticCall_level(level,varargin)
      ...
      [varargout{1:nargout}] = AVP.HW.AD.device.StaticCallbyName(fname{end},varargin{:});
    end % StaticCall_level

    function varargout = StaticCallbyName(name,varargin)
      %> calls DWF function with the same name as a calling member function
      %> form stack level LEVEL, e.g. level == 2 is immediate calling
      %> function
      % determine calling function name
      [status varargout{1:nargout}] = calllib('dwf',['FDwf' name],varargin{:});
      if status ~= 1
        error( AVP.HW.AD.device.GetLastErrorMsg());
      end
    end % StaticCall_level

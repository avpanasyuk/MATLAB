function varargout=CRC16_CCITT(bytes, do_not_reset, start_value)
  ...
  if nargin == 0
    ...
    for byteI = 1:numel(bytes)
      ui8LookupTableIndex = bitxor(bytes(byteI),uint8(bitshift(CRC16_CCITT_CRC,-8)));
      CRC16_CCITT_CRC = bitxor(CRC_ui16LookupTable(double(ui8LookupTableIndex)+1),...
        mod(bitshift(CRC16_CCITT_CRC,8),65536));
    end
    if nargout > 0, varargout{1} = uint16(CRC16_CCITT_CRC); end
  end
end

...
% use with m5pbuild and m5pcv functions.
%
% Call:
...
%                   each leaf will contain a linear regression model
%                   reduced in size using sequential backward selection
%                   algorithm.
...
%                   training observations) values, the tree will
%                   essentially behave like containing just one leaf
%                   (corresponding to the root node). For value 0, no
%                   smoothing is applied. (default value = 15 (Wang &
%                   Witten, 1997))
%   splitThreshold : A node is not split if the standard deviation of the
%                   values of output variable at the node is less than
%                   splitThreshold of the standard deviation of output
%                   variable for the entire training data (default value
%                   = 0.05 (Wang & Witten, 1997)). The results are usually
%                   not very sensitive to the exact choice of the threshold
%                   (Wang & Witten, 1997).
%
% Output:
%   trainParams   : A structure of training parameters for m5pbuild
%                   function containing the provided values (or default

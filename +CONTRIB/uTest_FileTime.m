...
% User Interface: --------------------------------------------------------------
% Do the work: =================================================================
disp(['===  Test CONTRIB.SetFileTime / GetFileTime  ', datestr(now, 0)]);
...
% Get file times:
disp('GetFileTime:');
Reply = CONTRIB.GetFileTime(File);
disp(Reply);

% DIR does apply FLOOR to the seconds:
fprintf('\n');
if isequal(round(datevec(FileDir.date)), floor(Reply.Write))
   disp('  ok: DIR and CONTRIB.GetFileTime reply the same WRITE time');
else
   error([ErrID, 'DIRneGetFileTime'], ...
      'DIR and CONTRIB.GetFileTime reply different WRITE time');
end
...
try
   ...
   CONTRIB.SetFileTime(Folder, 'Write',    theTime);
   CONTRIB.SetFileTime(Folder, 'Access',   theTime); % NTFS can delay change for 1 hour!
   CONTRIB.SetFileTime(Folder, 'Creation', theTime);
   
   FolderTime = CONTRIB.GetFileTime(Folder);
...
for iDate = 1:length(DateList)
   ...
   for iSpec = 1:length(SpecList)
      ...
      for iType = 1:length(TypeList)
         % Call CONTRIB.SetFileTime:
         aTimeType = TypeList{iType};
         try
            if isempty(aTimeType)  % Default:
               CONTRIB.SetFileTime(File, aSpec, aDate);
            else
               CONTRIB.SetFileTime(File, aSpec, aDate, aTimeType);
            end
         catch  % TRY-CATCH compatible with Matlab 6.5:
            error([ErrID, 'CrashedSet'], ...
               ['Crash in CONTRIB.SetFileTime (', aSpec, ', ', aTimeType ')', ...
                  char(10), lasterr]);
         end
         ...
         try
            if isempty(aTimeType)  % Default:
               ...
               Reply = CONTRIB.GetFileTime(File, aTimeType);
            end
         catch
            error([ErrID, 'CrashedGet'], ...
               ['Crash in CONTRIB.GetFileTime (', aTimeType ')', ...
                  char(10), lasterr]);
         end
         ...
         if isequal(Reply.(aSpec), aDate)
            ...
            error([ErrID, 'GetSetDiffers'], ...
               ['CONTRIB.GetFileTime(CONTRIB.SetFileTime(', aSpec, ', ', ...
         ...
         % Get a single time:
         S = CONTRIB.GetFileTime(File, aTimeType, 'struct');
         T = CONTRIB.GetFileTime(File, aTimeType, aSpec);
...
CONTRIB.SetFileTime(File, 'Write', [2011, 12, 1, 14, 15, 16.15], 'UTC');
aTime   = CONTRIB.GetFileTime(File, 'native');
...
CONTRIB.SetFileTime(File, 'Write',    i64Time, 'native');
CONTRIB.SetFileTime(File, 'Access',   i64Time, 'native');
CONTRIB.SetFileTime(File, 'Creation', i64Time, 'native');
aTime   = CONTRIB.GetFileTime(File, 'native');
...
testDate = clock;
try
   CONTRIB.SetFileTime([], 'Write', testDate);
   tooLazy = true;
catch
...
try
   CONTRIB.SetFileTime('this_is_a_not_existing_file%&%$', 'Write', testDate);
   tooLazy = true;
catch
...
try
   CONTRIB.SetFileTime(File, '_No_valid_spec', testDate);
   tooLazy = true;
catch
...
try
   CONTRIB.SetFileTime(File, 'Access', 'badate');  % String, but 6 chars
   tooLazy = true;
catch
...
try
   CONTRIB.SetFileTime(File, 'Access', 1:7);  % date with 7 elements
   tooLazy = true;
catch
...
try
   CONTRIB.SetFileTime(File, 'Access', zeros(1, 6));
   tooLazy = true;
catch
...
% Success! Goodbye: ------------------------------------------------------------
delete(File);
fprintf('\n== CONTRIB.SetFileTime and CONTRIB.GetFileTime passed the tests\n');

...
elseif strcmp(wildpath,'**')
  ...
  D = CONTRIB.rdir([prepath postpath(2:end)]);
  ...
  c_D = arrayfun(@(x) CONTRIB.rdir([prepath x.name filesep wildpath postpath]),...
  ...
  c_D = arrayfun(@(x) CONTRIB.rdir([prepath x.name postpath]),...
    D_sd, 'UniformOutput', false);
